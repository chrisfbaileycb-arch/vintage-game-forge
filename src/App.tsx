      ...
      <main>
        ...
        <section className="border-y border-white/10 bg-panel/70" aria-label="Live arcade activity">
          <div className="mx-auto grid max-w-6xl gap-px bg-white/10 sm:grid-cols-3">
            {LIVE_SIGNALS.map(({ icon: Icon, value, label }) => (
              <div key={label} className="flex items-center gap-4 bg-ink/80 px-6 py-5">
                <Icon className="h-5 w-5 text-neon" />
                <div>
                  <p className="font-pixel text-sm text-paper">{value}</p>
                  <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.18em] text-fog">{label}</p>
                </div>
